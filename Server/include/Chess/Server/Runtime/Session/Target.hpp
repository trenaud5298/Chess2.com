...
public:
    ...
    static Target Id(std::vector<std::uint64_t> targetIds) {return Target(IdPayload(std::move(targetIds)));}
    ...
    void forEach(const std::unordered_map<std::uint64_t, std::uint64_t>& idToSessionIndex, const std::vector<std::shared_ptr<Session>>& sessions, Func&& func) const {
        std::visit(overloaded{
            ...
            [&](const IdPayload& payload) {
                for (const std::uint64_t targetId : payload.targetIds) {
...
private:
    struct AllPayload{};
    struct IdPayload{std::vector<std::uint64_t> targetIds;};
